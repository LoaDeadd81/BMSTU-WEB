...
class RecipeTest {
    ...
    fun isNotOwner() {
        val actual = facade.isOwner(1u, 2u)
        val expected = false
...
//            listOf(IngredientInStage(1u, "name1", IngredientType.MEAT, 1u, 1u, ProcessingType.WASH))
//        )
//        facade.addStage(recipeId.toULong(), expected)
//
//        val tmp1 = transaction {
//            StageTable.findById(stageId) ?: throw NotFoundInDBException("Stage with id = $stageId not found")
//        }
//
//        val actual1 = tmp1.toEntity()
//        transaction { actual1.ingredients = tmp1.findIngredients() }
//
//        val tmp2 = facade.getRecipe(recipeId.toULong())
//        val actual2 = tmp2.stages.last()
//
...
class StageTest {
    ...
    private fun readStage(id: ULong): Stage {
        ...
        res.ingredients = transaction { dao.findIngredients() }
        return res
    }
    ...
    fun updateStage() {
        val id = 3
        val expected = Stage(
            id.toULong(), 3u, "desc3", listOf(
                IngredientInStage(3u, "name3", IngredientType.MEAT, 1u, 3u, ProcessingType.CLEAR),
                IngredientInStage(3u, "name3", IngredientType.MEAT, 2u, 3u, ProcessingType.DRY),
                IngredientInStage(3u, "name3", IngredientType.MEAT, 3u, 3u, ProcessingType.WASH)
            )
        )

        expected.description = "fixed1"
        facade.updateStage(expected)
        val actual = readStage(id.toULong())
    ...
    fun deleteStage() {
        val id = 3
        facade.deleteStage(id.toULong())
        assertThrows<NotFoundInDBException> {
            transaction {
                val obj = StageTable.findById(id)
                obj?.toEntity() ?: throw NotFoundInDBException("Stage with id = $id not found")
            }
        }
    }
...
class UserTest {
    ...
    fun createUser() {
        val id = 6
        val expected = User(id.toULong(), "login6", "password6", false)
        facade.createUser(expected)
        val actual = transaction {
            val obj = UserTable.findById(id)
    ...
    fun createUserNotUniq() {
        val tmp = User(99u, "login1", "password0", false)
    ...
    fun changeUserRole() {
        val id = 4
        val expected = true
        facade.changeUserRole(id.toULong(), true)
        val actual = transaction {
            UserTable.findById(id)?.toEntity()?.isAdmin ?: throw NotFoundInDBException("User with id = $id not found")
        }
